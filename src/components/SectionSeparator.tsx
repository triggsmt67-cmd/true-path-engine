...
const SectionSeparator: React.FC<SectionSeparatorProps> = ({ number, title }) => {
  return (
    <div className="relative w-full flex items-center justify-center py-12 md:py-16 overflow-hidden pointer-events-none z-20">
      ...
      <div className="absolute top-1/2 left-1/2 w-1/2 h-[1px] -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-transparent via-brand-accent/20 to-transparent blur-[1px]"></div>
      ...
      >
        <div className="flex items-center gap-3 px-4 py-2 rounded-standard bg-surface border border-black/5 shadow-lg shadow-black/[0.03]">
          <span className="text-[10px] font-bold text-brand-accent tracking-widest">
            {number}
          </span>
        ...
        <div className="absolute inset-0 bg-brand-accent/5 blur-xl rounded-full -z-10 opacity-50"></div>
      </motion.div>
