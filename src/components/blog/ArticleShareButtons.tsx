...
const ArticleShareButtons: React.FC<ArticleShareButtonsProps> = ({ title, slug }) => {
  ...
  const shareUrl = `https://www.truepath406.com/blog/${slug}`;
    ...
    <div className="flex items-center gap-3">
      {buttons.map((item) => (
        <button
          ...
          className="p-3.5 rounded-full border-2 transition-all group bg-surface border-black/5 text-text-secondary/60 hover:text-brand-accent hover:border-brand-accent/20 shadow-md hover:shadow-xl"
      ...
      <button className="p-3.5 rounded-full border-2 transition-all group bg-surface border-black/5 text-text-secondary/60 hover:text-brand-accent hover:border-brand-accent/20 shadow-md hover:shadow-xl">
        <Bookmark className="w-4 h-4 group-hover:fill-brand-accent transition-all" />
      </button>
    </div>
  );
};
