...
export async function POST(request: Request) {
  try {
    ...
    const { email, businessName, deficitScore, isPartial, location, websiteUrl, isVideoTeardownRequest } = await request.json();
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      let subject = isPartial 
        ? `Partial Lead (Gate Bounce Risk): ${safeSubjectName}` 
        : `New Trust Deficit Lead: ${safeSubjectName}`;
        
      if (isVideoTeardownRequest) {
        subject = `🎥 VIDEO TEARDOWN REQUEST: ${safeSubjectName}`;
      }

      const emailLayout = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
          <div style="background-color: ${isVideoTeardownRequest ? '#e0e7ff' : isPartial ? '#fef3c7' : '#dcfce7'}; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; border-bottom: 2px solid ${isVideoTeardownRequest ? '#4f46e5' : isPartial ? '#f59e0b' : '#22c55e'};">
            <h2 style="margin: 0; color: ${isVideoTeardownRequest ? '#3730a3' : isPartial ? '#92400e' : '#166534'};">
              ${isVideoTeardownRequest ? '🎥 Video Teardown Requested' : isPartial ? '⚠️ Partial Lead (Gate Bounce)' : '✅ Fully Converted Lead'}
            </h2>
